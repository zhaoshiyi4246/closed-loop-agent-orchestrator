...
- 当前唯一执行内容：**PR #45 / M4 AO v0.12.12 模型接入迁移返修，待审计；原 P01/P02 联合真实测试暂缓**。负责人扩大到全部 27 入口，并授权与自动 Worker 分层的手动原生终端。本轮新增实际目录/认证/终端接线、可展开选择器、新建前目录和不覆盖模型语义；非 Codex 自动执行桥仍未实现，不把终端启动计为完整闭环迁移。
- 已有 Kimi/GLM 配置与系统凭据保留；主目录用户未提交配置未改写。linked worktree 的默认配置来源改为同一主工作目录，详情与本轮验证见 P01 卡内 D11 记录。真实账号/套餐/模型/额度、完整 GUI 体验与发布验证仍 NOT_RUN。
...
- 当前停止条件：完成可审计返修与定向 Windows/浏览器检查、截图自查后更新现有 PR #45，等待审计；不自行合并，不使用真实 Key/模型，不将剩余自动执行桥或 M4 标为完成。
