...
- 唯一下一实施任务：**V03-P02 — Kimi 语义后端与切换评测**，TODO，尚未开始；先完成工程接入与离线验证，再按明确授权集中验证两家真实服务与角色准入。
...
- 当前停止条件：完成 PR #43 合并、背景文档推送与本地 main 同步后停止，不开始 P02。本轮沿用既有验证，仅检查文档链接与差异，不重跑测试、构建、smoke 或真实模型。配置保存、离线验证和代码审计均不等于真实 GLM 准入通过；真实测试要求保留，待两家工程接入完成后集中进行。完整 GUI 体验、全量、安装与发布验收尚未完成；“任务闭环运行视图”仍仅为未授权候选。
