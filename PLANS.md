...
> 计划稿：0.3-plan-r1，2026-09-06。负责人审核稿；文件尚未入库时，不代表任务已开始。
...
- 当前阶段：M0 — v0.3规划落盘。
- 当前任务：**V03-DOC-00**。
- 当前状态：TODO；等待负责人确认并将本次7个文件通过文档PR入库。
- 当前允许：文档/链接/路径/基线/产品blob核对。
- 当前禁止：修改产品代码、依赖、manifest/builder；模型调用、AO Worker、Mission、GUI、tag和发布。
- 下一项建议：**V03-F01 — 完整契约与终局一致性**。本项实施需要收到相应任务指令，不能因为列在“下一步”就自动开工。
...
| 阶段 | 状态 | 退出条件 |
|---|---|---|
| M0 规划落盘 | TODO | 负责人批准；文档入库；产品内容不变 |
...
新的A01—A12缺陷不被上述历史PASS清零。v0.3所有卡尚未实施；本次生成文档没有运行任何产品验收。
