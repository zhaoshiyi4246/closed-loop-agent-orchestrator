...
- 当前停止条件：完成 P02 工程接入、定向离线/浏览器验证并提交独立 PR 后停止，等待审计；不自行合并、不调用真实供应商。配置保存、离线验证和代码审计均不等于真实 GLM/Kimi 准入通过；真实测试要求保留，待两家工程接入完成后集中进行。完整 GUI 体验、全量、安装与发布验收尚未完成；“任务闭环运行视图”仍仅为未授权候选。
