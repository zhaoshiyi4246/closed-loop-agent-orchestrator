...
- 当前唯一任务：**V03-F03 — Git路径、产物规则与只读取证**，IN_PROGRESS；base `3af98d46e3495aa0154f8701152a11274b42a7c2`，分支 `codex/v03-f03-git-evidence`。GitHub API 已确认远端 main 与本地基线一致。
...
- 本轮授权：F03 Git 路径、产物规则、只读取证、基线完整性及直接相关消费者；实现后提交独立 PR 等待外部审计。
- F03 实测：Windows 定向 8 模块 **188 passed / 323.88s**；最终索引边界 4 模块 **110 passed / 162.69s**；Git 环境隔离与 materialization **21 passed / 23.22s**，均无失败/跳过；变更源码 compileall 通过。各集合重叠，不累计计数，命令与边界见 F03 卡。
...
- 下一步：F03 实现与定向检查已完成，提交 PR 后停止等待外部审计；F03 不提前标 DONE，不开始 F04。
...
| M1 修复冻结 | IN_PROGRESS（F01 / F02 DONE；F03 IN_PROGRESS） | F01—F05负例与正常路径通过 |
