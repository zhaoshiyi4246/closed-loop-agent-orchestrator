...
- 下一动作：Kimi managed-home已独立审查通过并提交e5c87e6；真实流式协议暴露审批前rawInput缺失，精确2.0.2参数解码已提交288e64d并经官方CLI离线三条实际消费者链路及独立审查通过（旧probe先后误判已纠正）。Windows Node ZIP深目录提取修正已提交1e1193b；新运行HEAD最终构建和实际安装双任务验收；真实有限对照已记录并独立核对，不再追加付费试次；总PR交接。
