...
- PR #46 当前返修：启动失败的持久请求独立于 Session 可见；按原生 owner 区分未启动与 UNKNOWN，关联回执丢失不重生 Worker，新尝试保留草稿并使用新身份/工作分支。检查与准确本机入口见 [原生证据](docs/reference/ao-native/README.md) / [开发说明](ao/CLAO.md)。迁移保持 IN_REVIEW，不标完整迁移完成。
- 当前停止条件：完成本次局部返修、开发启动与定向检查，更新原 PR #46 后等待再次审计；不合并，不追加其他迁移能力。
