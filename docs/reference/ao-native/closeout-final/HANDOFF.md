...
该候选已实际安装并通过单/双任务、六组显示、断连与独立导出Gate验收；6462个安装文件逐项校验。卸载测试退出0、163个任务/证据文件与官方AO保持不变。测试安装已卸载，交付目录中的安装器可供负责人安装。安装器自身退出码因辅助等待超时未采集，文件完整性与实际运行均通过，不补造exit0。详见[最终安装证据](installed-1325591/README.md)。
