...
**独立项目入口（U02 先行，尚未实现）。** 目标是用户无需预先使用或配置 AO，能在 CLAO 内打开/创建项目并执行任务；普通本地项目不要求 GitHub/origin。先完成独立项目入口与本地执行，再完成任务旅程。优先复用成熟编码引擎，Codex App Server 是候选；具体执行适配在后续专门切片落实，U01 不改当前 AO 后端，也不通过改名伪装本地目录能力。
...
| D10 | 2026-09-07：U01 收敛为日常工作台，开发夹具退出产品；U02 先独立项目/本地执行，再完整旅程；Q01 独立安装与有限对照/角色消融 | 负责人 PR #39 返修授权。当前 AO 后端不变，成熟引擎（Codex App Server 候选）的执行适配另行切片；不要求普通本地项目具有 GitHub/origin；历史 M0/M1/M2 完成状态不变，新增目标未实现 |
