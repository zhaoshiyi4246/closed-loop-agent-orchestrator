...
| D10 | 2026-09-07：U01 收敛为日常工作台，开发夹具退出产品；U02 先独立项目/本地执行，再完整旅程；Q01 独立安装与有限对照/角色消融 | 负责人 PR #39 返修授权；2026-09-08 U02 首切片进一步授权采用本机 Codex 0.150.1 App Server stdio 执行 Worker，历史 AO 显式兼容；不要求普通本地项目具有 GitHub/origin；历史 M0/M1/M2 完成状态不变；首切片已再次外部审计 PASS 并合入，真实模型运行仍 NOT_RUN，不等于 U02 整卡或独立安装完成 |
