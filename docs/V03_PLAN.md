...
本稿不是新一轮全量代码审计，也没有运行 AO、真实模型、Windows 回归或 GUI。2026-09-06 读取 GitHub 时，主仓库 main 仍为上述基线。本稿中所有 v0.3 接口、状态、设计尺寸和验收目标均是**新设计**，需要实施与验证后才可写入产品说明。[S02]
...
浏览器自动化放开发测试环境，不进入产品 runtime 依赖。设计先以固定状态夹具和截图审核，再接真实 Store 数据；不能只提交一张漂亮静态页面。
...
| M5 发布候选 | V03-Q01 | 新Windows包、负例／恢复／GUI／每家模型受控live、来源／校验与发布说明 |
...
| 发布 | 最终ZIP自身bootstrap、自测、link/hash/hygiene；两套干净Windows环境；人工GUI |

负例集中的假成功、越界自动批准、无回执副作用必须为 0。UI 控制请求本地 ACK 建议 p95 <1s、事件可见建议 p95 <2s；这不承诺外部 AO／模型终止同步完成。阶段耗时、失败率、配置和费用未知项需如实显示。

供应商比较使用报告建议的同任务基线：最终目标 20 个代表性任务、每个候选配置至少 3 次；可先小样本筛查，再经用户批准预算执行完整矩阵。对比时固定任务、base、Gate、范围和成功标准；成功与失败的耗时分开，不能用快速报错拉低平均值。速度改善是待测结果，不是选 GLM/Kimi 的既定事实。[S01，第 23 页]
...
新的 PROJECT/PLANS 可以变短，不再附带全部 R0—R5 日志。原始完整文件继续由冻结 `v0.2` tag 和固定提交链接保存；新文件提供入口。不得重写旧证据为全问题清零。旧源码目录保留但不进入产品包；本轮不归档／删除旧仓库，也不改发布tag。
...
这次只提交治理与规划文件，不改 `交付/closed-loop-v2/**`、manifest 或 builder，不发模型调用、不执行 Mission。建议分支 `docs/v03-plan-r1`，一个文档 PR 合入。

入库后先让 Codex做 V03-DOC-00：核对文件、链接、当前基线、产品 blob 未变，发现矛盾只报告；随后由规划审计助手给 **V03-F01：完整契约校验与终局一致性** 的实施指令。允许在该任务中有条件吸收队友补丁，但不直接 Sync fork／整目录覆盖，不沿用队友 Linux 离线结果为新 Windows验收。
