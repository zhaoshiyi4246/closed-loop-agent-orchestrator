...
**2026-09-09 新授权优先：** 当前迁移为“AO 原生底座 + CLAO 闭环”（7.6 / D11）。以下旧阶段设计与完成记录保留为历史，不能据此宣称新底座已验收；P01/P02 联合真实测试暂缓。
...
### 7.6 AO 原生底座迁移（负责人 2026-09-09 授权）

2026-09-10 收尾：PR #46 启动失败返修通过外部代码审计并 rebase 合入；“AO 原生底座 + 单 Worker 验收闭环基础集成” DONE，整体迁移与 M4 仍 IN_PROGRESS。后续 **Planner/Auditor 异常决策与独立角色配置迁移，DONE**，PR #47 再次代码审计 PASS 并于同日 rebase 合入（范围见本节末）；当前切片 **运行恢复与用户指令回执迁移，IN_REVIEW**。任意在途执行恢复、旧历史/连接导入、普通目录/未提交来源支持、独立导出与结果中心、闭环运行图及正式发布入口仍未迁移；历史阶段完成不代表新底座已验收。既有 Windows/Electron 与离线证据沿用，完整体验、真实账户/模型及发布验收尚未完成，Codex `account_storage_unsafe` 保持待解决。

以 AO v0.12.12 完整原生前后端为可复核基线，受控放入 `ao/`，保留源码目录、原始导入提交、许可证与归属。PR #45 的旧 Panel 执行器/模型页翻译路线被替代，不再追加或合并；已保存配置、凭据及历史均保留。

原生项目、模型目录/搜索/默认、账号、Session、Chat/终端、文件及执行器由 AO 直接提供。仅增加 opt-in 闭环任务、验收/结果展示，不再复制模型选择器，不嵌入旧 Panel，不复制云服务、移动端或 SCM 平台。完整入口保留不等于所有执行器已通过闭环准入；Chat、终端能力、平台缺依赖按实际能力区分。

AO 管进程/Session/workspace，CLAO 增量唯一调度其闭环 Worker，AO 自动 follow-up 不同时拥有该 Worker。Mission/operation/验收事实沿用同一 SQLite 扩展点，事务绑定 immutable owner；复用 Python 纯逻辑，不启动第二 Panel/Controller。保留停止未知不交付、范围/Gate 不被模型 PASS 覆盖、结构化 AC 复核、有界修复及原项目不自动写回。

开发版使用独立应用身份、端口、数据/运行发现位置，无官方自动更新目标；不连接、关闭、迁移或修改重装后的官方 AO。旧 CLAO 数据如需迁移须明确导入，不混用数据库。正式默认入口在对应集成验收后再切换。

本轮退出条件为真实开发构建/原生桌面启动、原生模型菜单操作、真实 AO Session/SQLite/Git 下的正常通过与明确 Gate 失败、一次修复及非 Codex 代表路径；仅外部模型/进程边界用替身。旧能力未迁移项与客观平台阻塞单列，源码导入/离线通过不等于全角色兼容或真实计费验收。

#### 原生角色决策迁移（2026-09-10，本轮授权）

PR #46 基础切片后的单 Worker 角色决策与独立配置已由 PR #47 再次代码审计 PASS 并合入（DONE）；冻结选择贯通实际 Chat 启动/恢复。完整迁移/M4 仍 IN_PROGRESS，当前切片为运行恢复与用户指令回执迁移（IN_REVIEW）。四角色沿用 AO 原生选择器并在创建时解析/冻结；缺少原生只读能力的执行器不用于自动语义角色，普通 AO 能力不变。角色/incident/operation 继续保存在同一 AO Mission 文档，以精确 owner 管理独立 Session 和取消。正常路径 gate-first，故障才诊断/规划，复用旧五动作契约；当前单 Worker 已停止后的 CONTINUE 是一次观察/复查，不重启，持续失败进入 HUMAN。受限替换从冻结来源重新执行原约束下的新方案，保留旧 Session/证据，不属于多任务分解或并行。更完整恢复、旧数据导入与图形运行视图不在本切片；开发能力不等于真实账户或全执行器验收。

#### 原生运行恢复与指令回执（2026-09-10 授权）

仅迁移当前单 Worker 路径。AO Mission 文档内维护下一未完成阶段、输入摘要、原角色响应、动作计数及指令/消费者关联；不用旧 StateStore 或第二调度器。启动对账/历史查看不派发，用户显式继续才取得既有 owner。复用已确认回合/角色结果与固定产物，尚未执行动作沿用原身份和预算；Gate/固定产物本身也可能有副作用，结果缺失不能任意重跑。无法确认的原生在途执行保留 UNKNOWN 与停止处理，终态仅新尝试。

原生 Worker Chat 与闭环输入统一持久接收和 Session/回合身份；语义要求冻结到下一次必要角色输入，Planner 镜像消费独立于主目标。审批/回答仍走 AO 原请求，不混作指令；补充文本不能变更权限/AC/Gate。具体恢复阶段、已验证边界与原生限制见 ao/CLAO.md。本切片 IN_REVIEW，整体迁移/M4 IN_PROGRESS，未迁移目标不裁剪，不提前启动下一任务。
...
| D11 | 2026-09-09：AO v0.12.12 原生底座 + CLAO 闭环；替代 PR #45 | 负责人最新迁移授权；独立开发身份/数据，真实开发构建必需；旧阶段状态保留为历史，不代表新底座验收；P01/P02 联合真实评测暂缓；详见 7.6 |
