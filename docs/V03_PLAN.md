...
本稿不是新一轮全量代码审计，也没有运行 AO、真实模型、Windows 回归或 GUI。规划起草时采用上述发布源码；规划文件已进入 main `3a9ea27468915eb9571611bcca10962e7a732fb0`，与发布源码的产品 blob 差异为 0。本稿中所有 v0.3 接口、状态、设计尺寸和验收目标均是**新设计**，需要实施与验证后才可写入产品说明。[S02]
