...
继续既有 AO v0.12.12 原生 Electron/React/Go + CLAO 增量与 PR #49，不换底座。单一总PR完成外部审计返修、简洁原生旅程、账户安全路径、GLM-5.3标准API与Kimi/Codex真实准入、安装候选和最终验收。本轮授权替代旧阶段“真实测试/发行后置”，费用/请求/Worker上限见PLANS；不静默跨服务，不把标准API宣称套餐计费。

默认任务详情按任务操作、执行/待处理、结果/改动/验收组织；一张总流程表现双分支及最终复核，Gate与语义事实分开，高级证据按需查看。复用原生选择器和组件，不新增控制面。唯一发布映射切换至自含运行依赖的原生候选入口，旧Panel/Controller退出发行运行路径。Q01两套干净Windows、实际产物旅程、有限对照与最终独立审查仍是退出门；未满足保持RELEASE_CANDIDATE或BLOCKED，不替负责人签署体验或发布。
...
**任务闭环运行视图（2026-09-08 候选；2026-09-11 纳入收官大阶段）。** 负责人提出高亮真实运行的组件，点击节点查看对应任务的动作、证据、耗时和输出，以理解本次执行与修复过程。后续可考虑复用现有阶段、事件、Gate 和复核记录形成轻量只读视图，不新增 Agent、控制层、工作流编辑器或用户样例模式。此项不属于 PR #41 返修，不改变已完成卡的状态或既定任务顺序；当前唯一下一任务以 PLANS.md 为准。2026-09-11 已授权只读运行高亮、真实节点证据与 Session 定位；不做编排器或额外模型调用。
...
#### 原生迁移收官大阶段（2026-09-11）

PR #48 已外部审计 PASS 并合入（DONE）。唯一执行指针为原生迁移收官大阶段 IN_REVIEW；整体迁移/M4 IN_PROGRESS。负责人授权同一阶段补齐本地来源、最多两个独立子任务、结果中心和固定版本独立导出、显式旧历史/配置/连接衔接、轻量只读运行图与统一启动/操作旅程。保留同一闭环所有者、冻结配置、已确认动作复用、UNKNOWN 不盲重发和停止确认，不新增控制面。内部 Coordinator 协调 Implementer 与独立 Reviewer，修复复核后统一阶段 PR/外部审计。真实服务/费用、全量及发行验收后置授权；不触碰用户配置与官方 AO。

实现边界（本阶段）：分解限定最多两个无依赖、路径与 AC 可分离的子任务；共享总修复/替换额度，子任务 DONE 仅表示本子任务验收，统一集成后才作 Mission 最终结论。旧配置内容版本与连接身份分离，明确触发导入，不覆盖原引用；标准 API 语义消费者与原生执行器保持分层。结果包只取已固定对象，运行图只读当前记录，不参与调度。
