...
func OpenFileNoFollow(path string) (*os.File, error) {
	handle, info, ownerCurrent, _, aclSafe, err := OpenWindowsPath(path, false, true)
	if err != nil {
		return nil, err
	}
...
func ValidateDirectory(path string, requirePrivate bool) error {
	handle, info, ownerCurrent, _, aclSafe, err := OpenWindowsPath(path, true, requirePrivate)
	if err != nil {
	...
	return ValidateDirectoryAncestors(path)
}

func ValidateDirectoryAncestors(path string) error {
	...
	for current := abs; ; current = filepath.Dir(current) {
		...
		handle, info, _, ownerTrusted, aclSafe, openErr := OpenWindowsPath(current, true, false)
		if openErr != nil {
		...
		if verifiedChild && WindowsLocalVolumeRoot(current, handle) {
...
func WindowsLocalVolumeRoot(path string, handle windows.Handle) bool {
...
func OpenWindowsPath(path string, directory, requirePrivate bool) (windows.Handle, windows.ByHandleFileInformation, bool, bool, bool, error) {
	return OpenWindowsPathWithAccess(path, directory, windows.GENERIC_READ|windows.READ_CONTROL, requirePrivate)
}

func OpenWindowsPathWithAccess(path string, directory bool, access uint32, requirePrivate bool) (windows.Handle, windows.ByHandleFileInformation, bool, bool, bool, error) {
...
func ProtectPrivateDirectory(path string) error {
	handle, _, ownerCurrent, _, _, err := OpenWindowsPathWithAccess(
		path,
		true,
...
func ProtectPrivateFile(path string, file *os.File) error {
	if file == nil {
	...
	handle, opened, ownerCurrent, _, _, err := OpenWindowsPathWithAccess(
		path,
		false,
		windows.WRITE_DAC|windows.READ_CONTROL,
		false,
...
func SyncDirectory(path string) error {
	handle, info, ownerCurrent, _, aclSafe, err := OpenWindowsPathWithAccess(path, true, codexWindowsDirectoryFlushAccess(), false)
	if err != nil {
		return err
	}
	defer windows.CloseHandle(handle)
