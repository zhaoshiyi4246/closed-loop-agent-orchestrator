...
-- with the full 0052 column/index/CDC shape and only widens status.
...
CREATE TABLE conversation_activities_new (
    ...
    status            TEXT NOT NULL CHECK (status IN ('running', 'completed', 'failed', 'cancelled', 'pending', 'resolved')),
...
-- +goose Down
-- +goose StatementBegin
-- Existing cancelled rows cannot fit the old CHECK, so downgrade keeps the
-- widened constraint, matching the additive-status policy above.
SELECT 1;
-- +goose StatementEnd
