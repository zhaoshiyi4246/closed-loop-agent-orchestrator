...
-- The session preference is part of the session read model. Recreate the
-- current trigger from migration 0078 with this field in both its change
-- predicate and payload so the existing CDC/SSE invalidation path refreshes
-- the toggle immediately. The PR review-thread triggers remain valid after the
-- additive ALTER TABLE above and continue to invalidate SCM summaries when a
-- thread is first observed or resolved.
-- +goose StatementBegin
...
-- +goose StatementBegin
ALTER TABLE pr_comment DROP COLUMN auto_inject_review;
ALTER TABLE pr_reviews DROP COLUMN auto_inject_review;
ALTER TABLE review_run DROP COLUMN auto_inject_review;
ALTER TABLE sessions DROP COLUMN auto_inject_review;
-- +goose StatementEnd
