...
(async () => {
  ...
  try {
    ...
    if (process.env.CLAO_TEST_CLOSEOUT === '1') {
      ...
      keepCompletedWindow = process.env.CLAO_DESKTOP_KEEP_OPEN === '1';
      return;
    }
    ...
    const start = async (objective) => {
      ...
      if (process.env.CLAO_TEST_ROLES === '1') {
        const details=page.getByText('复核模型与修复设置',{exact:true});
        await details.click();
...
with sqlite3.connect(sys.argv[1]) as db:
 ...
 BEGIN SELECT RAISE(ABORT, 'injected receipt write failure'); END""")`, path.join(home,'data/ao.db')]);
      ...
      await page.getByRole('button', { name: '打开执行会话', exact: true }).waitFor();
    ...
    if (process.env.CLAO_TEST_ROLES === '1') {
      ...
      await page.getByText('高级详情',{exact:true}).click();await page.getByText('角色与决策',{exact:true}).click();
    ...
    await page.getByText('CLAO · 验收通过', { exact: true }).waitFor({ timeout: 60000 });
    await page.getByRole('region', { name: '验收摘要' }).waitFor();
    ...
    throw error;
  } finally { if (keepCompletedWindow) { console.log('COMPLETED_DESKTOP_LEFT_OPEN',home); setInterval(()=>{},10000); } else await app.close(); if (holder) await new Promise(resolve => holder.close(resolve)); }
