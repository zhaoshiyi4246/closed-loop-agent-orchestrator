...
describe("installed CLAO acceptance runtime", () => {
	it("fails closed for an incomplete installation and resolves a self-contained install with spaces", () => {
		...
		try {
			...
			for (const file of ["python/python.exe", "clao-core/src/loopcore/ao_acceptance.py", "clao-core/src/loopcore/ao_legacy.py"]) {
				mkdirSync(path.dirname(path.join(root, file)), { recursive: true });
				writeFileSync(path.join(root, file), "fixture");
			}
			expect(packagedCoreEnvironment(root)).toEqual({
				CLAO_CORE_PYTHON: path.join(root, "python", "python.exe"),
			...
			expect(env.CLAO_CORE_PYTHON).toBe(path.join(root, "python", "python.exe"));
