...
M4 首切片将模型页合并为服务/账号方式、模型与角色：保留 Codex/GLM/Kimi 历史，接入 Codex 账号/API、GLM-5.3 标准 API 与官方 Claude Code Coding Plan 语义路径，支持公开目录和自定义模型；Worker 仍为 Codex。连接和参数冻结，系统凭据按服务隔离；尚未迁移的 AO 入口及真实登录/计费/效果均不宣称完成，见[模型连接说明](clao/README.md#模型连接与角色)。
