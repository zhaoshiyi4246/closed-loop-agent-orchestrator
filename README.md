...
P01 已合入 BigModel 通用服务 `glm-4.7` 的语义角色配置、Windows 系统凭据与 HTTP 传输；Planner 分解/异常调用、Auditor、Mission Verifier 可分别选择 Codex/GLM；本轮 P02 分支加入 Kimi 国内通用服务 `kimi-k3`，共用现有模型页与传输，角色/凭据/外发许可按服务隔离，待审计。Worker 仍为 Codex。配置保存、离线验证与代码审计通过不等于真实 GLM 请求及角色准入已通过；真实测试保留并后置集中进行，当前支持与配置边界见 [产品说明](clao/README.md#glm-语义角色配置p01-工程切片)。
