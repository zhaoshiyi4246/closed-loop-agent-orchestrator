...
M4 / PR #45 将模型页收敛为服务或执行器 → 认证 → 可浏览/搜索的模型菜单 → 角色。保留 Codex/GLM/Kimi 历史，支持 Codex 账号/API、Claude 账号/API、GLM-5.3 标准 API、官方 Claude Code Coding Plan 与 Kimi 国内语义路径；27 个 AO 基准工具另有明确分层的手动原生终端。自动 Worker 仍为 Codex，非 Codex 结构化执行桥尚未完成，不把终端层计为全部闭环迁移。运行选择冻结，系统凭据不迁移；真实登录/套餐计费/模型效果仍未验证，见[模型连接说明](clao/README.md#模型连接与角色)。
