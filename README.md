...
U03 在既有任务详情提供固定结果的变更、差异和逐项验收，以及复制路径、打开目录和独立 ZIP 补丁包。包生成后可脱离原工作树使用；仅支持明确的文本/文件模式范围，历史缺版本或含不支持/禁止材料时不伪造完整导出。用法与限制见[结果中心说明](clao/README.md#结果中心与独立补丁包v03-u03)。本轮仍是离线 Git/HTTP/浏览器验证，不是发布验收。
