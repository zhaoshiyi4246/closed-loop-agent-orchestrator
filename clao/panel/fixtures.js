...
function previewSnapshot(name){
  ...
  if(["cancelling","cancelled","stop_unknown"].includes(name)){
    ...
    m.cancellation={status:name==="cancelled"?"confirmed":name==="stop_unknown"?"unknown":"requested",reason:m.reason};
  }
  const s={mission:m,running:["running","approval","cancelling","disconnected"].includes(name),elapsed:84,missions:[{mission_id:m.id,objective:m.objective,state:m.state}],subtasks:[{task_id:"TASK-SAMPLE-01",objective:"校验输入格式，保留正常导入行为",state:name==="success"?"DONE":"WORKER_RUNNING",worker_session_id:"SESSION-SAMPLE-01",local_fixes:0,max_local_fixes:2,replans:0,max_replans:1}],panel_errors:[],read_errors:[],alerts:[],transitions:[],verifications:[],traffic:[],config:{poll_seconds:5,idle_audit_cooldown_seconds:120,blocked_escalation_seconds:600,l0_nudge_grace_seconds:120},default_config:snapshot,mission_config:{status:"ok",snapshot},config_fields:{},directive_receipts:{status:"ok",records:[]},gate_query:{status:"not_run",records:[]},phases:{status:"ok",records:[{sequence:1,mission_id:m.id,task_id:"TASK-SAMPLE-01",phase:({approval:"approval_wait",success:"verifier",failure:"final_gate",cancelling:"kill",cancelled:"kill",stop_unknown:"reconciliation",gate_read_error:"final_gate"})[name] || "worker_execution",status:["running","approval","cancelling","disconnected"].includes(name)?"running":"completed",reason:m.reason,attempt:1,elapsed_seconds:32}],roles:{},workers:[{session_id:"SESSION-SAMPLE-01",requested_model:"gpt-5.6-sol",passed_model:"gpt-5.6-sol",spawn_resolved_model:"gpt-5.6-sol",spawn_model_evidence:"AO SessionView.model (sample)",activity:"working",usage:null,cost:null}]}};
